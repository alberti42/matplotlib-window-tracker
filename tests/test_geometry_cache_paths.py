...
def test_resolve_cache_dir_explicit(monkeypatch: Any, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("MATPLOTLIB_WINDOW_TRACKER_CACHE_DIR", raising=False)
    d = geometry_cache._resolve_cache_dir(tmp_path)
    assert d == tmp_path / ".matplotlib-window-tracker"


def test_resolve_cache_dir_env(monkeypatch: Any, tmp_path: Path) -> None:
    from matplotlib_window_tracker import geometry_cache

    monkeypatch.setenv("MATPLOTLIB_WINDOW_TRACKER_CACHE_DIR", str(tmp_path))
...
def test_resolve_cache_dir_script_uses_entry_dir(
    ...
    d = geometry_cache._resolve_cache_dir(None)
    assert d == script_dir / ".matplotlib-window-tracker"
